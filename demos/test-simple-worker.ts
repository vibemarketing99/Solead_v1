...
/**
 * Demo script to test SimpleStagehandWorker functionality
 * Run with: npx ts-node demos/test-simple-worker.ts
 */
...
async function demo() {
  console.log('🚀 SimpleStagehandWorker Demo\n');
  ...
  try {
    ...
    // Step 5: Initialize SimpleStagehandWorker pool
    ...
    // Query and display created tasks
    const tasks = await taskRepo.find({
      relations: ['lead'],
      where: { lead: { agentId: testAgent.id } },
      order: { createdAt: 'DESC' },
      take: 5
    });
    
    console.log(`\n📋 Sample Tasks (${tasks.length} most recent):`);
    tasks.forEach((task, index) => {
      console.log(`   ${index + 1}. Task ${task.id.substring(0, 8)}...`);
      console.log(`      Status: ${task.status}`);
      console.log(`      Lead: @${task.lead.authorHandle}`);
    });
    ...
    console.log('\n' + '='.repeat(50));
    console.log('✨ SimpleStagehandWorker Demo completed successfully!\n');
    ...
    console.log('   ✅ Task creation for approval workflow');
    console.log('   ✅ Pool scaling capabilities confirmed');
    console.log('\n📌 Next Steps:');
    console.log('   1. Fix TypeScript issues in automation files');
